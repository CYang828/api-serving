...
class UsersRepository(BaseRepository):
    async def get_user_by_phone(self, *, phone: str) -> UserInDB:
            user_row = await queries.get_user_by_phone(self.connection, phone=phone)
            if user_row:
                return UserInDB(**user_row)

            raise EntityDoesNotExist("user with phone {0} does not exist".format(phone))
    ...
    async def create_user(
        ...
        *,
        ...
        user = UserInDB(username=username, email=email, phone=phone, wechat=wechat)
    ...
    async def update_user(  # noqa: WPS211
        ...
        *,
        ...
        user_in_db.wechat = wechat or user_in_db.wechat
